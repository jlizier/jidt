 ...
 *  or a Ventage Point Tree for multi-dimensional / multiple variables.
 ...
 */
public abstract class PeriodicNearestNeighbourSearcher 
     ...
     */
    public static PeriodicNearestNeighbourSearcher create(double[][] data)
    throws Exception {
     ...
     *  by time, second index by variable number.
     */
    public static PeriodicNearestNeighbourSearcher create(double[][] data, double[] boundary)
    throws Exception {
        if ((data == null) || (data[0].length == 0)) {
            return null;
        }else if (data[0].length == 1) {
            // We have univariate data:
//            return new UnivariatePeriodicNearestNeighbourSearcher(MatrixUtils.selectColumn(data, 0), boundary[0]);
        	return new VpTree(data,boundary);
        } else {
//            Change to VPTree.
            return new VpTree(data,boundary);
        }
    ...
    findKNearestNeighbours(int K, int sampleIndex,
    	...
    	if (dynCorrExclTime != 0) {
			throw new UnsupportedOperationException("dynamic correlation exclusion time is not "
					+ "implemented in Periodic Boundary Conditions");
		}
		return findKNearestNeighbours(K, sampleIndex); 
    }
